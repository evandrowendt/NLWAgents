...
const model = 'gemini-2.5-flash' //versão da AI
...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcreva o áudio para PT-BR. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em paragrafos quando for apropriado.',
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  const prompt = `
    Com base no texto fornecido abaixo como contexto, responda a pergunta de forma clara e precisa em PT-BR.
